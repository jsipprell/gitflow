...
if [ -z "$REPO_HOME" ] ; then
	REPO_HOME="http://github.com/jsipprell/gitflow.git"
fi

EXEC_FILES="git-flow"
SCRIPT_FILES="git-flow-init git-flow-feature git-flow-hotfix git-flow-release git-flow-support git-flow-version git-flow-deploy gitflow-common gitflow-shFlags"
